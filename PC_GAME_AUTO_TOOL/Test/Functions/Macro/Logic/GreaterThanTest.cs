...
namespace Test.Functions.Macro.Logic
{
    ...
    public class GreaterThanTest
    {
        ...
        public void testGreaterThan()
        {
            // trueとなることを確認する
            {
                PC_GAME_AUTO_TOOL.Functions.Macro.Logic.InterFace.Logic command = new GreaterThan(1, "-1", "-2");
                Assert.IsTrue(command.execute());
            }

            // trueとなることを確認する
            {
                PC_GAME_AUTO_TOOL.Functions.Macro.Logic.InterFace.Logic command = new GreaterThan(1, "0", "-1");
                Assert.IsTrue(command.execute());
            }

            // trueとなることを確認する
            {
                PC_GAME_AUTO_TOOL.Functions.Macro.Logic.InterFace.Logic command = new GreaterThan(1, "1", "0");
                Assert.IsTrue(command.execute());
            }

            // falseとなることを確認する
            {
                PC_GAME_AUTO_TOOL.Functions.Macro.Logic.InterFace.Logic command = new GreaterThan(1, "1", "1");
        ...
        public void testGreaterThan3Args()
        {
            ...
            Assert.ThrowsException<ArgumentException>(() =>
            {
                PC_GAME_AUTO_TOOL.Functions.Macro.Logic.InterFace.Logic command = new GreaterThan(1, "1", "2", "3");
            });
        }
        ...
        public void testGreaterThanWrongArgs()
        {
            ...
            Assert.ThrowsException<ArgumentException>(() =>
            {
                PC_GAME_AUTO_TOOL.Functions.Macro.Logic.InterFace.Logic command = new GreaterThan(1, "a", "2");
            });

            // コンストラクタで例外がスローされることを確認する
            Assert.ThrowsException<ArgumentException>(() =>
            {
                PC_GAME_AUTO_TOOL.Functions.Macro.Logic.InterFace.Logic command = new GreaterThan(1, "1", "b");
            });
        }
    }
}
